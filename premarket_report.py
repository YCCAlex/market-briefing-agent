...
- Guards to only run at 09:00 US/Eastern on weekdays
...
def get_ai_report(model, earnings: List[str], winners: List[Dict], losers: List[Dict]) -> str:
    ...
    prompt = f"""
你是專業美股分析師，負責撰寫給投資研究主管的每日美股盤前早報。
現在時間為美東時間上午 9:00，請根據以下已提供的結構化資料，生成一份專業、精煉、格式完全一致的盤前早報。

已提供資料：
今日預計公布財報公司：{earnings_today}
盤前大幅波動公司（盤前強勢股）：{winners}
盤前大幅波動公司（盤前疲弱股）：{losers}

引用規則：
盤前強勢股/疲弱股段落中，「參考資料連結」必須逐行列出該檔股票 news 欄位中的 url（若 url 為空或 news 為空，則不輸出）；不得自行編造或補上任何網址。
